...
form.addEventListener('submit', async (e) => {
  ...
  if (ultimoEnvio) {
    ...
    if (tempoPassado < tempoEspera) {
      alert(
        "⏳ Você já enviou um currículo recentemente. Por favor, aguarde 5 minutos para enviar novamente.",
      );
      return;
    }
  }
  ...
  try {
    ...
    const { data: urlCv } = clienteSupabase.storage
      .from("arquivos")
      .getPublicUrl(nomePdf);
    ...
    const { data: urlVideo } = clienteSupabase.storage
      .from("arquivos")
      .getPublicUrl(nomeVideo);
    ...
    msgStatus.innerText =
      "Erro: " + (erro.message || "Falha ao enviar. Tente novamente.");
